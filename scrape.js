...
RenderUrlsToFile = function(top_urls, callbackPerUrl, callbackFinal) {
    ...
    initiate = function(){
    	total_urls = top_urls.length;
    	if (u_limit == 0 || u_limit > total_urls) {
	    	u_limit = total_urls;
	    }
    	...
    	if (folder_path == null){
    		// path = 'doctoralia_data_'+urls[0].split("/")[5].split("-")[0]+'_'+rand+'.json';
    		path = 'doctoralia_data_'+rand+'.json';
    	} else {
    		// path = "./"+folder_path+'/doctoralia_data_'+urls[0].split("/")[5].split("-")[0]+'_'+rand+'.json';	
    		path = "./"+folder_path+'/doctoralia_data_'+rand+'.json';
    	}
		console.log(urls[0]);
		data_parsed = false;
					...
					function handle_click_reaction(page){
						data = page.evaluate(function(){
							if ($(".title h1").text() != ""){
								...
								// Reviews & answers
								var reviews = $(".contribution .reviews").text().trim();
								var answers = $(".contribution .answers").text().trim();
								...
								// Services & facilities
								var services = $(".services #servicesList > li").map(function(){return $('> span.name', this).text()+" - "+$('> span.price b', this).text()}).get().join(", ");
								if (services == ""){
									// Facilities
								...
								// Languages
								var languages = $("section.languages ul li").map(function(){return $(this).text()}).get().join(", ");

								// Doctor Registration Number
								var reg_num = $("section.history p.regnum").text().replace(/.*\s([0-9]*).*/,'$1');

								// Additional Photos
								var clinicPhotos = $(".slider li").map(function(){return $('a img',this).attr('src')}).get().join(", ");
			...
			return page.open(url, function (status) {
				if (status == "success"){
					...
					var pagination_count = page.evaluate(function(){
						var uniquePages = {};
						$("div.paging ul a").map(function(){
							if ($(this).attr("href")) {
								uniquePages[$(this).attr("href")] = 1;	
							}
						});
						return Object.keys(uniquePages).length;
					});
					var loops = page.evaluate(function(){
			        	return $("div.paging ul a").map(function(){
